...
from conftest import STUB_USAGE, FakeProviderModel, ScriptedModel, canned_checklist
...
def test_llm_judge_scores_met_fraction_and_carries_usage():
    ...
    assert [c.met for c in result.criteria] == [True, True, False]
    assert all(c.reasoning for c in result.criteria)


def test_llm_judge_tolerates_a_markdown_code_fence():
    wrapped = f"```json\n{canned_checklist([True, True, True])}\n```"
    judge = LLMJudge(model=ScriptedModel(wrapped), rubric_version="v1")
...
def test_llm_judge_repairs_a_malformed_response():
    ...
    model = ScriptedModel(bad, canned_checklist([True, False, True]))
    judge = LLMJudge(model=model, rubric_version="v1", max_repair_attempts=1)

    result = judge.score(ToyTask(), [Message(role="assistant", content="x")], seed=0)

    assert result.score == pytest.approx(2 / 3)
    assert result.usage == Usage(input_tokens=2, output_tokens=2, cost_usd=2e-6, latency_s=0.02)
...
def test_llm_judge_rejects_transcript_not_ending_with_assistant():
    ...
    judge = LLMJudge(model=ScriptedModel(canned_checklist([True, True, True])), rubric_version="v1")
